...
# Launch Janitor EC2 Instance Script
# This script launches a new EC2 instance using the Janitor launch template
...
echo "🚀 Launching Janitor EC2 instance..."
...
RESULT=$(aws ec2 run-instances \
  ...
  --min-count=1 \
  --max-count=1 \
  --tag-specifications="ResourceType=instance,Tags=[{Key=Name,Value=janitor-$ENV-runner},{Key=Project,Value=janitor-$ENV},{Key=Environment,Value=$ENV}]" \
  --output=json)

# Extract instance ID
INSTANCE_ID=$(echo "$RESULT" | jq -r '.Instances[0].InstanceId')
INSTANCE_STATE=$(echo "$RESULT" | jq -r '.Instances[0].State.Name')
