...
export const readFileContent = async (
  ...
  try {
    ...
    if (offset > 0 || limit > 0) {
      ...
      const endIndex =
        limit > 0 ? Math.min(lineCount, startIndex + limit) : lineCount;
...
export const listDirectory = async (
  ...
  try {
    ...
    console.log(
      `Listing directory: ${resolvedPath}, recursive: ${recursive}, depth: ${depth}`
    );
...
export const editFileTool = createTool({
  ...
  inputSchema: z.object({
    ...
    content: z
      .string()
      .nullable()
      .describe("New content (or null to delete the file)"),
    createIfNotExists: z
      .boolean()
      .optional()
