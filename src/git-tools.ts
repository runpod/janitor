  ...
  try {
    ...
    if (fs.existsSync(targetPath)) {
      ...
      if (pullResult.success) {
        ...
        throw new Error(
          `Failed to pull latest changes: ${pullResult.errorMessage}`
        );
      }
    } else {
      // Clone the repository
      console.log(
        `Repository doesn't exist at ${targetPath}, cloning from ${repoUrl}`
      );
      ...
      if (cloneResult.success) {
        ...
        const errorOutput =
          cloneResult.stderr || cloneResult.errorMessage || "";
